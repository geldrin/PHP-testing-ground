 ...
 * MODIFIED BY DotsAmazing.com
 * changes at: https://github.com/sztanpet/plupload
 */
	...
	plupload.runtimes.Html5 = plupload.addRuntime("html5", {
		...
		init : function(uploader, callback) {
			...
			uploader.bind("Init", function(up) {
				...
				if ( up.settings.mimetypes )
					mimes = up.settings.mimetypes;
			...
			uploader.bind("UploadFile", function(up, file) {
				...
				function sendBinaryBlob(blob) {
					...
					// DAM resume support
					if (file.startFromChunk) {
						chunk  = file.startFromChunk;
						loaded = chunk * settings.chunk_size;
					}
					...
					function uploadNextChunk() {
						...
						function prepareAndSend(bin) {
							...
							if (up.settings.multipart && features.multipart) {
								...
								if (typeof(bin) !== 'string' && !!window.FormData && !$j.browser.opera) {
									formData = new FormData();
	
									formData.append('size', file.size ); // DAM change, size argument
								...
								if (typeof(bin) === 'string') {
									...
									// DAM change, size argument
									multipartBlob += dashdash + boundary + crlf +
										'Content-Disposition: form-data; name="size"' + crlf + crlf;
	
									multipartBlob += unescape(encodeURIComponent(file.size)) + crlf;
