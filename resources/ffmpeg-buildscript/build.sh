...
. ./env.source
    ...
    #../fetchurl "http://www.ffmpeg.org/releases/ffmpeg-2.3.3.tar.bz2"         # ffmpeg stable
    ../fetchurl "http://ffmpeg.org/releases/ffmpeg-snapshot-git.tar.bz2"       # ffmpeg git-snapshot
# packages for custom build:
    git clone git://git.ffmpeg.org/rtmpdump                                    # rtmpdump original (ffmpeg.org hosted)
    ../fetchurl "http://downloads.sourceforge.net/project/opencore-amr/vo-aacenc/vo-aacenc-0.1.3.tar.gz"
    ../fetchurl "https://www.openssl.org/source/openssl-1.0.1l.tar.gz"
    ../fetchurl "http://www.wavpack.com/wavpack-4.70.0.tar.bz2"
...
# Custom, with librtmp:
CFLAGS="-I$TARGET_DIR/include" LDFLAGS="-L$TARGET_DIR/lib -lm" ./configure --prefix=${OUTPUT_DIR:-$TARGET_DIR} --extra-cflags="-I$TARGET_DIR/include -static" --extra-cflags=--static --extra-ldflags="-L$TARGET_DIR/lib -lm -static" --extra-libs="-ldl" --extra-version=static --disable-debug --disable-shared --enable-static --extra-cflags=--static --disable-ffplay --disable-ffserver --disable-doc --enable-gpl --enable-pthreads --enable-postproc --enable-gray --enable-runtime-cpudetect --enable-libfaac --enable-libmp3lame --enable-libopus --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-bzlib --enable-zlib --enable-nonfree --enable-version3 --enable-libwavpack --enable-libvpx --enable-libvo-aacenc --enable-librtmp

# Custom, _without_ librtmp:
# CFLAGS="-I$TARGET_DIR/include" LDFLAGS="-L$TARGET_DIR/lib -lm" ./configure --prefix=${OUTPUT_DIR:-$TARGET_DIR} --extra-cflags="-I$TARGET_DIR/include -static" --extra-ldflags="-L$TARGET_DIR/lib -lm -static" --extra-version=static --disable-debug --disable-shared --enable-static --extra-cflags=--static --disable-ffplay --disable-ffserver --disable-doc --enable-gpl --enable-pthreads --enable-postproc --enable-gray --enable-runtime-cpudetect --enable-libfaac --enable-libmp3lame --enable-libopus --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-bzlib --enable-zlib --enable-nonfree --enable-version3 --enable-libvpx --enable-libvo-aacenc
