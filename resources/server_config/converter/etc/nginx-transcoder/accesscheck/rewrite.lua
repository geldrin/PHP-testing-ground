...
do
  ...
  if arguments.sessionid then
    ...
    ngx.log(ngx.DEBUG, ngx.var.uri, ' sessionid param found, OK')
...
local matches = ngx.re.match( ngx.var.uri, regex, 'jo')
if not matches then
  ngx.log(ngx.DEBUG, ngx.var.uri, ' unknown url, forbid')
...
if not arguments.sessionid then
  ...
  ngx.log(ngx.DEBUG, ngx.var.uri, ' no sessionid in encoded filename found, forbid')
  return ngx.exit(ngx.HTTP_FORBIDDEN)
end
ngx.var.sessionid = arguments.sessionid
ngx.req.set_uri_args('')
...
ngx.log(ngx.DEBUG, 'rewritten url from: ', ngx.var.uri, ' to: ', newuri )
