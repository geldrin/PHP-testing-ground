<li class="listingitem">
  <div class="row">
    <h3><a href="{$language}/channels/details/{$item.id},{$item.title|filenameize}">{$item.title|escape:html}{if $item.subtitle} <span class="subtitle">{$item.subtitle|escape:html}</span>{/if}</a></h3>
    <ul class="actions">
