...
class element {
  ...
  function replacePlaceholders( 
    ...
    if ( strtolower( $this->type ) == 'fieldset' ) // nonstandard layout
      $layoutused = $this->layout;
    if ( $this->rowlayout ) // override higher level defaults
  ...
  function applyElementInjection( $html, $layout, $errorstyle, $showerroricon, $erroricon ) {

    if ( preg_match_all( '/(\<FORM.([^>]+)\>)/msU', $html, $results ) ) {
      ...
      foreach ( $results[1] as $key => $result ) {
        ...
        $html = str_replace( 
          $result, 
          $element->getHTMLRow( $layout, $errorstyle, $showerroricon, $erroricon ),
          $html 
        );
