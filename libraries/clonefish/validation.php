...
class validation {
  ...
  // -------------------------------------------------------------------------
  function getJSValue( &$element = null ) {

    if ( $element )

      return 'clonefishGetFieldValue( ' . 
        '"' . $this->form->name . '", ' . 
        '"' . $element->getRealName() . '", ' .
        '"' . $element->type . '"' . 
      ')';
    ...
    if ( !strlen( $jscode ) )
      ...
      foreach ( $items as $item ) {
        ...
        foreach ( $results[1] as $key => $result ) {
          ...
          $getJSValue = '( ' . $this->getJSValue( $element ) . ' )';

          $item['js'] = str_replace( $result, $getJSValue, $item['js'] );
