...
class selectDate extends element {
  ...
  var $null         = Array( '' => '' );
    ...
    //
    // This element is only used for displaying the date selects.
    // The value returned depends on your format string, and will
    // always be filled with zeroes. With a format string like 
    // '%Y-%M-%D' you'll get '0000-00-00' in the end if you don't
    // select a proper date.
    //
    // Don't use array keys like Array( 23 => '' ) here as it will
    // corrupt the element because 23 could also be a date too. 
    // Use invalid date syntax instead: keys like '', 0 or 'foo' are all 
    // great.

  var $nullvalue     = null;
  var $nullbehaviour = 'zeroes';
  // Determines what the returned value should be when null option
  // was selected. Use 'zeroes' for returning zeroes 
  // (eg. '%Y-%M-%D' becomes '0000-00-00') or 'nullvalue' for returning
  // $this->nullvalue.
  //
  // 0000-00-00 dates are convenient 
  // database values instead of NULLs, which are
  // converted to Nov 30, -0001 by PHP.
  ...
  function setValue( $value, $magic_quotes_gpc ) {
    ...
    if ( class_exists( 'DateTime' ) ) {
      ...
      if (
          preg_match( '/^([^\d]|0)+$/', $value ) || ( $value === null ) ||
  ...
  function createStoredFormat( $year, $month, $day, $hour, $min, $sec, $magic_quotes_gpc ) {
    ...
    $nullvalue =
      ...
      ( is_array( $this->null ) && count( $this->null ) )
    ;
    ...
    if (
      ...
      if ( $nullvalue ) {
        switch ( $this->nullbehaviour ) {
          case 'nullvalue': $this->value = $this->nullvalue; break;
          default:          $this->value = $out; break;
        }
      }
      else
        $this->value = $out;
