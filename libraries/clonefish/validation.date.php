...
class dateValidation extends validation {
  ...
  function getJSCode( ) {
    ...
    $code .= 'errors.addIf( \'' . $this->element->_getHTMLId() . '\', ' .
      ...
      'clonefishValidDateTime( ' .
        ...
        "'" . @$this->element->nullbehaviour . "', " .
        "'" . @$this->element->nullvalue . "' " .
      ') ' .
      ", \"" .
  ...
  function isValid() {
    ...
    if (
         (
           !$this->required && 
           ( strtolower( $this->element->type ) == 'selectdate' ) &&
           ( $this->element->nullbehaviour == 'nullvalue' ) &&
           ( $this->element->getValue( 0 ) == $this->element->nullvalue )
         )
         ||
         (
           !$this->required &&
    ...
    $indexes = Array(
      ...
      'days'  => strpos( $this->format, 'D' ) !== false ? strpos( $this->format, 'D' ) : strpos( $this->format, 'Z' ),
    ...
    if ( 
         !$this->required &&
         ( $year + $month + $days + $hour + $min + $sec == 0 )
       )
      return $this->results;
    ...
    $zeroday = strpos( $this->format, 'Z' ) !== false ? true : false;

    if (
         ( ( $month !== false ) && ( ( $month < 1 ) || ( $month > 12 ) ) ) ||
         ( !$zeroday && ( $days !== false ) &&  ( ( $days <= 0 ) || ( $days > 31 ) ) ) ||
         ( $zeroday && ( $days !== false ) &&  ( ( $days < 0 ) || ( $days > 31 ) ) ) ||
         ( 
           $month &&
